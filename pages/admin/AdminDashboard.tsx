...
// --- START: UTILITY & CHART COMPONENTS (unchanged) ---
...
const BarChart: React.FC<BarChartProps> = ({ chartData, title, barColor = 'text-brand-primary' }) => {
...
// API stats type (same shape your API returns)
type DashboardStats = {
  deposits: {
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Use stats if present; show "Loading…" while null
  const loadingStats = dashboardStats == null;

  // Fallbacks (if dashboardStats not yet loaded)
  const totalUsers = dashboardStats?.users.totalUser ?? users.length;
  const activeUsers = dashboardStats?.users.activeCount ?? users.filter(u => u.walletBalance > 0).length;
  const inactiveUsers = dashboardStats?.users.inactiveCount ?? users.filter(u => u.walletBalance <= 0).length;

  const totalApprovedDepositsAmt = dashboardStats?.deposits.approved.totalAmount ?? 0;
  const totalApprovedDepositsCount = dashboardStats?.deposits.approved.count ?? 0;

  const totalApprovedWithdrawalsAmt = dashboardStats?.withdrawals.approved.totalAmount ?? 0;
  const totalApprovedWithdrawalsCount = dashboardStats?.withdrawals.approved.count ?? 0;

  const pendingDepositsCount = dashboardStats?.deposits.pending.count ?? 0;
  const pendingDepositsAmt = dashboardStats?.deposits.pending.totalAmount ?? 0;

  const pendingWithdrawalsCount = dashboardStats?.withdrawals.pending.count ?? 0;
  const pendingWithdrawalsAmt = dashboardStats?.withdrawals.pending.totalAmount ?? 0;

  // Charts still rely on arrays you pass in
  const chartData = useMemo(() => {
    ...
    switch (period) {
      ...
      case 'weekly':
        ...
        break;
      case 'monthly':
        startDate = startOfMonth(subDays(startOfMonth(now), 150)); // ~6 months back
        interval = 'month';
        for (let i = 5; i >= 0; i--) {
    ...
    const processData = (items: (DepositRequest | WithdrawalRequest | User)[]) => {
      ...
      items.forEach(item => {
        const dateSource = 'joinDate' in item ? item.joinDate : (item as any).date;
        ...
        if (itemDate >= startDate && itemDate <= endDate) {
          ...
          if (index >= 0 && index < aggregatedData.length) {
            aggregatedData[index] += 'amount' in item ? (item as any).amount : 1;
          }
        }
      });
    ...
    return {
      ...
      newUsers: processData(users.map(u => ({ ...u, date: u.joinDate } as any))),
    };
  ...
  const userStatusData = useMemo(() => ({
    ...
    data: [
      { label: 'Active', value: activeUsers, color: 'stroke-green-400' },
      { label: 'Inactive', value: inactiveUsers, color: 'stroke-red-400' },
    ],
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              {loadingStats ? (
                ...
                <>
                  <p className="text-3xl font-bold text-white">${fmtMoney(totalApprovedDepositsAmt)}</p>
                  <p className="text-xs text-gray-400 mt-1">{totalApprovedDepositsCount} approvals</p>
                </>
              )}
            </Card>
            ...
            <Card>
              ...
              {loadingStats ? (
                ...
                <>
                  <p className="text-3xl font-bold text-white">${fmtMoney(totalApprovedWithdrawalsAmt)}</p>
                  <p className="text-xs text-gray-400 mt-1">{totalApprovedWithdrawalsCount} approvals</p>
                </>
              )}
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
              onClick={() => onNavigate('deposits', RequestStatus.PENDING)}
            ...
            >
              <Card>
                ...
                {loadingStats ? (
                  ...
                  <>
                    <p className="text-3xl font-bold text-brand-primary">{pendingDepositsCount}</p>
                    <p className="text-xs text-gray-400 mt-1">Amount: ${fmtMoney(pendingDepositsAmt)}</p>
                  </>
                )}
              </Card>
            </div>
            ...
            <div
              onClick={() => onNavigate('withdrawals', RequestStatus.PENDING)}
              className="cursor-pointer transition-transform duration-200 hover:-translate-y-1"
            >
              <Card>
                ...
                {loadingStats ? (
                  ...
                  <>
                    <p className="text-3xl font-bold text-brand-primary">{pendingWithdrawalsCount}</p>
                    <p className="text-xs text-gray-400 mt-1">Amount: ${fmtMoney(pendingWithdrawalsAmt)}</p>
                  </>
                )}
              </Card>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div onClick={() => onNavigate('users', 'all')} className="cursor-pointer transition-transform duration-200 hover:-translate-y-1">
              <Card>
                ...
                {loadingStats ? <p className="text-sm text-gray-500">Loading…</p> :
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex space-x-2 rounded-lg bg-brand-surface p-1">
              {(['daily', 'weekly', 'monthly', 'custom'] as Period[]).map(p => (
                <button key={p} onClick={() => setPeriod(p)} className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${period === p ? 'bg-brand-primary text-white' : 'text-gray-300 hover:bg-gray-700'}`}>
            ...
            {period === 'custom' && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="bg-gray-800 border border-gray-600 rounded-md p-1.5 text-sm"
                />
                <span>to</span>
                <input
                  type="date"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <BarChart chartData={chartData.withdrawals} title="Approved Withdrawals" barColor="text-brand-primary" />
        </div>
      </div>
    </div>
  );
